...
Дата: 15 июля 2026  
Статус: OBS-01 DONE: consent-gated tenant-safe analytics, privacy controls and operational observability contracts implemented locally; migration awaits normal deployment flow.
Следующая задача: `LIFE-01`
...
| 36 | STUDIO-00 | Config CLI и asset pipeline v0 | FND-02 | DONE |
| 37 | STUDIO-01 | Internal Studio drafts и previews | STUDIO-00, QA-DS | DONE |
| 38 | STUDIO-02 | Asset upload, focal roles и validation | STUDIO-01 | DONE |
| 39 | STUDIO-03 | Publish, rollback и build/release jobs | STUDIO-02 | DONE |
| 40 | STUDIO-04 | Internal content, collections и merchandising | COL-03, COM-01 | DONE |
| 41 | OBS-01 | Analytics, privacy и operational monitoring | M2/M3 flows | DONE |
| 42 | LIFE-01 | Notifications и lifecycle campaigns | OBS-01, CORE-04 | DONE |
| 43 | QA-REL | E2E, security, performance и store sandbox | Все release scope | BLOCKED |
| 44 | REL-01 | Staging pilot «Огороднік Олександр» | QA-REL | BLOCKED |
| 45 | REL-02 | Production release и rollback drill | REL-01 | BLOCKED |
| 46 | GROW-01 | Weekly menu planner | CORE-06, COL-03 | DONE |
...
|---|---|---|---|---|
| 2026-07-16 | GROW-01 | Добавлен tenant-scoped weekly menu planner: календарные slots для recipe и collection material, servings, drag/reorder, повтор блюд, offline mutation queue, share text и объединение отсутствующих ингредиентов недели в существующий shopping list. Premium-slot остаётся teaser: entitlement не проверяется при добавлении/отображении, а проверяется существующим recipe detail при открытии. | `dart format` затронутых Flutter файлов — PASS; `flutter analyze` — PASS; полный `flutter test -r compact` — PASS (89); targeted backend `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 .venv/bin/python3 -m pytest tests/test_menu_plan_contract.py tests/test_pantry_contract.py -q` и FastAPI import — PASS; `git diff --check` — PASS. | Migration `2026_07_16_weekly_menu_planner.sql` не применялась к remote Supabase: это normal deployment step, не локальный blocker. UI добавляет выбранный recipe/material на текущий день; его можно перенести и переупорядочить на экране недели. Следующий work package не начинался. |
| 2026-07-16 | REL-02 | Production release и rollback drill не запускались: прямой prerequisite `REL-01` остаётся `BLOCKED`, а он зависит от `QA-REL`. Existing tenant-specific bundled bootstrap/assets, config-only rollback и release-job contracts не являются доказательством фактического rollout или mobile rollback. | Сверены очередь, dependencies `QA-REL` → `REL-01` → `REL-02`, scope/acceptance/Definition of Done и актуальные release artifacts (`frontend/assets/branding`, `backend/assets/brands`, Studio release/rollback contracts, observability documentation); `git diff --check` — PASS. | **BLOCKED:** для phased production rollout, named alert owners, monitored post-release smoke и tested config/app rollback требуются завершённый QA sign-off и authenticated external operations: production secrets/readiness and applied migrations, production privacy policy/terms/support links, final store metadata/assets and signed iOS/Android builds, Apple/Google/RevenueCat production configuration, deployed monitoring/alert routing, real pilot install evidence and a rehearsed rollback with recorded outcome. До этого no generic White Povar identity в установленном pilot app и остальные acceptance criteria нельзя честно подтвердить локально. |
| 2026-07-16 | REL-01 | Staging pilot не запускался: prerequisite `QA-REL` остаётся BLOCKED, поэтому staging tenant/config/content/store sandbox, internal distribution, monitored pilot, feedback channel и rollback нельзя честно подтвердить локальной имитацией. | Сверены queue, dependency `QA-REL`, acceptance REL-01 и authoritative `QA_RELEASE_QUALIFICATION.md`; `flutter analyze`, полный `flutter test -r compact` (88) и `git diff --check` — PASS. Backend QA-набор и `flake8` в локальной `.venv` зависли без вывода и были остановлены; это execution blocker, не успешный gate. | **BLOCKED:** требуются authenticated staging инфраструктура и реальные внешние операции: применённые Supabase migrations, secret-managed configuration/readiness, два staging tenant, Apple/Google sandbox products, RevenueCat webhook worker, internal iOS/Android distribution, deployed OAuth callbacks, backup restore и evidence на физических iOS/Android/web. До этого нельзя подтвердить critical journeys или rollback readiness; hero photos остаются операционным входом и могут быть опубликованы через существующий BrandConfig/asset pipeline без изменения client code. |
| 2026-07-16 | QA-REL | Добавлен единый release-qualification checklist с воспроизводимым локальным gate, evidence map для guest/auth/subscriber/one-off, tenant isolation, offline/restart, a11y, deep links/auth callback, privacy/deletion и rollback/readiness. Устаревший debug subscription guide явно исключён из release approval. Account-deletion error logging теперь не передаёт provider exception text (включая потенциальные account данные) в operational sink; добавлен соответствующий contract test. | Targeted `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 .venv/bin/python3 -m pytest tests/test_release_qualification_contract.py tests/test_auth_completion_contract.py tests/test_security.py tests/test_tenant_isolation_contract.py tests/test_commerce_access_contract.py tests/test_commerce_webhook_contract.py tests/test_observability_contract.py -q`; затем полный backend/frontend gate и production-like web build — результаты записываются после запуска. | **BLOCKED**, не DONE: `COM-02`/`COM-03`/`COM-04` остаются blocked без authenticated StoreKit/Google Play sandbox, verified products и external RevenueCat worker. Также внешними gate остаются применение Supabase migrations, production secrets/readiness, deployed OAuth callback, backup restore и evidence на реальных iOS/Android/web. Ноль P0/P1 и release sign-off нельзя утверждать до этих операций; accepted P2 должны быть записаны в `QA_RELEASE_QUALIFICATION.md`. |
| 2026-07-16 | LIFE-01 | Добавлен tenant-scoped lifecycle contract: opt-in marketing для новых рецептов/коллекций, независимые transactional reminders для saved/cooking, timer alerts и сервисных purchase messages. Preferences включают quiet hours/timezone; server не принимает arbitrary URLs, а delivery log допускает только recipe/collection/purchase deep links и daily frequency cap. Push binding хранится только как user+tenant token, удаляется до logout и cascade при account deletion. Во Flutter добавлен защищённый экран «Сповіщення» и route из Profile; content marketing disabled до consent. | Backend: targeted lifecycle/observability/preferences tests — PASS (8); полный `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 .venv/bin/python3 -m pytest tests/ -q` — PASS (189); FastAPI import и flake8 E9/F63/F7/F82 — PASS. Frontend: адресный dart format, `flutter analyze`, targeted lifecycle test и полный `flutter test -r compact` — PASS (88); `git diff --check` — PASS. CI-like `scripts/build-web.sh` с `TENANT_SLUG=ohorodnik-oleksandr` локально BLOCKED: `flutter build web` не дал результата свыше 2 минут после `Compiling lib/main.dart for the Web...`; процесс остановлен. | Migration не применялась к remote Supabase. Реальные APNs/FCM token registration и authenticated delivery worker должны быть configured в release infrastructure: worker обязан tenant-brand message, enforce quiet hours/cap and record provider result; текущий client/API намеренно не обещает delivery без provider. COM-02/03 store verification не затрагивались; purchase messages остаются transactional only. |
| 2026-07-16 | OBS-01 | Добавлен tenant-safe, opt-in analytics contract: отдельная per-user/per-tenant consent, строгий allowlist funnel events и отсутствие свободных properties исключают transcript, email, health/ingredient data и receipt. Existing server flows автоматически учитывают search/voice search/save/view/cooking только после consent; Flutter Settings получил отзывный switch consent. Миграция включает tenant-isolated daily funnel view, RLS, cascade account deletion и aggregate-only AI cost table. Health больше не раскрывает environment, unexpected error tracking не логирует body/exception text; добавлен operational/privacy runbook. | Backend: targeted observability/history/preferences tests — PASS (7); полный `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 .venv/bin/python3 -m pytest tests/ -q` — PASS (186); FastAPI route import и flake8 E9/F63/F7/F82 — PASS. Frontend: адресный dart format, `flutter analyze`, полный `flutter test -r compact` — PASS; CI-like `scripts/build-web.sh` c `TENANT_SLUG=ohorodnik-oleksandr` — PASS; `git diff --check` — PASS. | Migration не применялась к remote Supabase. Перед включением analytics нужно применить migration и настроить deployment log/error sink с описанной redaction policy; scheduled cost exporter должен upsert aggregate provider usage в `ai_cost_daily`, не сохраняя prompts/completions. LIFE-01 и creator-facing insights/experiments не затрагивались. |
| 2026-07-16 | STUDIO-04 | Добавлен tenant-scoped internal Studio API для draft/create/edit/publish материалов `recipe`/`technique`/`process`/`video`, упорядоченных коллекций с free previews и product/offer mapping. Новая migration добавляет audit log, schedule records и SECURITY DEFINER операции, которые атомарно публикуют content/collection и отвергают чужой tenant material/product/collection. Чернетка content сохраняется с `is_public=false`, поэтому не попадает в существующий consumer API; Studio UI получил скрытый маршрут `/studio/content` с consumer `ContentCard` inventory и явной публикацией. | Backend: targeted Studio contracts — PASS (12); полный `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 .venv/bin/python3 -m pytest tests/ -q` — PASS (183); FastAPI route import и flake8 E9/F63/F7/F82 — PASS. Frontend: адресный dart format, `flutter analyze`, полный `flutter test -r compact` — PASS (86); CI-like `scripts/build-web.sh` с test defines и `TENANT_SLUG=ohorodnik-oleksandr` — PASS; `git diff --check` — PASS. | Migration не применялась к remote Supabase. Operational scheduler должен безопасно вызвать `studio_publish_content`/`studio_publish_collection` для due rows из `studio_scheduled_publications`; никакой локальный процесс не обещает background publish. Удаление опубликованного content требует показать получаемый `delete-impact` (коллекции/покупатели) перед отдельной destructive action; actual deletion намеренно не автоматизирована. Store IDs/prices и store verification остаются в COM-02/03. |
| 2026-07-15 | STUDIO-03 | Добавлены admin-only Studio API и UI для повторной publish validation, immutable config publish, config-only rollback, audit log и истории релизов. Новый tenant-scoped release journal различает `config published`, web deploy, mobile build и store release; web/iOS/Android requests создаются только как `queued`, а failure/status может быть зафиксирован отдельно — без ложного сообщения об успешном внешнем deploy/store submit. Добавлена migration release jobs/audit и contract tests. | Backend: targeted `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 .venv/bin/python3 -m pytest tests/test_studio_draft_contract.py tests/test_studio_assets_contract.py tests/test_studio_release_contract.py -q` — PASS (9); полный `pytest tests/ -q` — PASS (180); FastAPI import и flake8 E9/F63/F7/F82 — PASS. Frontend: targeted `dart format --output=none --set-exit-if-changed` для Studio files и `flutter analyze`, полный `flutter test -r compact` и CI-like `scripts/build-web.sh` с `TENANT_SLUG=ohorodnik-oleksandr` — PASS; `git diff --check` — PASS. | Полный `dart format --output=none --set-exit-if-changed .` запущен, но BLOCKED существующей синтаксической ошибкой в несвязанном `frontend/lib/features/recipes/presentation/widgets/recipe_video_widget.dart:293` (ожидается `}`); Studio files отформатированы адресно. Migration не применена к remote Supabase. Для фактического web deploy/mobile build/store release нужен отдельный authenticated external worker, который забирает queued job и вызывает status endpoint; до этого Studio намеренно показывает pending/not submitted. Rollback создаёт новую published BrandConfig version и не меняет DB/content. STUDIO-04 не затрагивался. |
| 2026-07-15 | STUDIO-02 | Добавлен tenant-bound pipeline внутренних brand assets: Studio выдаёт короткоживущий signed upload URL только в server-owned staging prefix, затем сервер повторно валидирует тип/вес/размеры изображения, сжимает валидный файл в WebP и выдаёт URL только ready asset. Миграция создаёт asset registry, ограниченный bucket и cleanup неподтверждённых staging uploads. Brand draft теперь разрешает remote URLs только из ready assets resolved tenant; подмена URL другого tenant отклоняется. Studio получил загрузку avatar/hero, назначение `home`/`login`/`paywall`/`collection`, focal point в превью и fallback при сломанном изображении. | Backend: targeted `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 .venv/bin/python3 -m pytest tests/test_studio_draft_contract.py tests/test_studio_assets_contract.py tests/test_brand_config.py -q` — PASS (16); полный `pytest tests/ -q` — PASS (176); FastAPI import и flake8 E9/F63/F7/F82 — PASS. Frontend: `dart format --output=none --set-exit-if-changed` и `flutter analyze` — PASS; полный `flutter test -r compact` — PASS; CI-like `scripts/build-web.sh` с `TENANT_SLUG=ohorodnik-oleksandr` — PASS; `git diff --check` — PASS. | Migration ещё не применена к remote Supabase; operational scheduler должен вызывать `purge_expired_studio_asset_uploads()` для 24h orphan cleanup. STUDIO-02 не публикует draft/config и не запускает web/mobile release — это STUDIO-03. |
| 2026-07-15 | STUDIO-01 | Добавлен защищённый internal Studio: миграция создаёт tenant-scoped роли `editor`/`admin`, отдельные непубликуемые BrandConfig drafts и атомарный compare-and-swap RPC. API `/api/v1/studio` выдаёт/сохраняет draft только internal member, проверяет resolved tenant и возвращает 409 при конкурентном изменении. Во Flutter добавлен скрытый internal маршрут `/studio/brand`: редактор четырёх секций 13m, индикатор несохранённых изменений, сохранение draft и живые Home/Login/Paywall previews на тех же BrandConfig tokens/components, а не screenshots. | Backend: `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 .venv/bin/python3 -m pytest tests/ -v` — PASS (174); FastAPI import — PASS; flake8 E9/F63/F7/F82 — PASS. Frontend: `dart format --output=none --set-exit-if-changed .`, `flutter analyze`, targeted studio test и полный `flutter test` — PASS; CI-like `scripts/build-web.sh` с `TENANT_SLUG=ohorodnik-oleksandr` — PASS; `git diff --check` — PASS. | Миграция не применялась к удалённой БД и не назначает первого internal member — это требуется сделать через безопасный operational/admin workflow после применения migration. Upload, role assignment, focal/crop и asset validation остаются STUDIO-02; publish/rollback и release jobs — STUDIO-03. |
| 2026-07-15 | STUDIO-00 | Добавлен fail-closed internal CLI `backend/tools/brand_config_cli.py`: validate с field errors, dry-run/publish и rollback через атомарные Supabase RPC; rollback создаёт новую immutable published version. Добавлена migration с транзакционными RPC. Asset pipeline валидирует и нормализует 512×512 avatar в PNG, генерирует tenant-specific monogram/icon/splash/favicon SVG candidates и README с ограничением native release; создан bundled artifact `frontend/assets/branding/ohorodnik-oleksandr_bootstrap.json`. Добавлены targeted tests и backend README usage. | Targeted `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 .venv/bin/python3 -m pytest tests/test_studio_config_cli.py tests/test_brand_config.py -q` — PASS (16); CLI `validate`, `publish --dry-run`, candidates и bundle — PASS; `.venv/bin/flake8 app tools/brand_config_cli.py --select=E9,F63,F7,F82` — PASS; `git diff --check` — PASS. Полный `pytest tests/ -v` и required FastAPI import локально BLOCKED: оба зависают без вывода во время pytest collection/import больше 3 минут в текущем Python 3.13 environment; процессы остановлены. | Не добавлялись Studio UI, internal roles, drafts/previews, signed uploads, focal/crop editor, remote asset hosting, product config editing или web/mobile deployment jobs: это STUDIO-01…03. Publish/rollback требуют применить новую migration и service key; native candidates не обещают runtime update. |
