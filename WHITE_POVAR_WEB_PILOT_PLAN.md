...
|---|---|---|---|---|
| 2026-07-16 | DEPLOY-01 | **BLOCKED до production mutation.** Предварительные gates пройдены: `DB-02` завершена, backup/recovery evidence и DB postflight зафиксированы в предыдущей записи; точный кодовый кандидат для deploy — `65bae0a1ba07cef8b8f9bb37bea715d3f8636241`. Никаких push, Render deploy, изменения env vars или миграций не выполнялось. | Read-only deploy dry-run в Render workspace `My workspace` — PASS: services/URLs/repository/branch подтверждены. Оба сервиса используют `main` и `autoDeploy=yes`, `trigger=checksPass`: API `white-povar-api-p79r`, web `white-povar-p79r`. Предыдущие healthy rollback baselines: API `dep-d9b4pqeq1p3s73eqeer0` (commit `f239e0c…`), web `dep-d9b5au2b0ehc73bipuk0` (commit `7a75b029…`). | Push в текущей topology начнёт API и web одновременно и нарушит обязательный API → smoke → web порядок. Нужна явная авторизация владельца на один из вариантов: перевести web в manual promotion в Render dashboard, либо разрешить concurrent backward-compatible rollout. Также текущий `HEAD` содержит несвязанный commit `f85ab56`; перед push approved `65bae0a` нужен clean release checkout/branch, чтобы не отправить пользовательскую работу. Следующая задача остаётся `DEPLOY-01`; после решения повторить preflight и только затем deploy. |
