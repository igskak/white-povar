...
|---|---|---|---|---|
| 2026-07-19 | QA-01 | **BLOCKED: первый frontend P1-fix deployed, но post-reload buyer smoke выявил race восстановления auth.** Activation создаёт один active server entitlement и paywall корректно показывает Premium. После full reload premium collection всё ещё locked: initial entitlement read случался до готовности persisted auth session. Follow-up `da5cf2a` связывает paywall refresh с `authProvider`: authenticated session запускает server entitlement read, sign-out clears in-memory access; добавлены isolated test overrides. Follow-up pushed to `main`; повторная browser-проверка была выполнена до подтверждения нового static deploy и всё ещё увидела старый locked state. Active entitlement сразу отозван. | Production preflight `/health` и `/health/ready` = 200; browser activation/entitlement drill воспроизвёл P1 после web deploy `8bca13f`; server audit подтвердил `1 active`, затем CLI revoke = `1`. Local follow-up: targeted Flutter tests `22 passed`, `flutter analyze`, release `flutter build web --release` и `git diff --check` PASS. | Подтвердить Render static deploy SHA `da5cf2a`, затем повторить buyer activation → full reload → premium recipe body → revoke/reload. `QA-01` остаётся BLOCKED также на remaining 17-journey evidence, Studio/free-account flows, one-off offer/scope и reversible config/web rollback drill. Не переходить к `SHOW-01`. |
