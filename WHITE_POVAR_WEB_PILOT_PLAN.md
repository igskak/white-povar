...
Следующая задача: `DEPLOY-01` (BLOCKED: требуется явное решение по Render web auto-deploy для API-first promotion)
...
| 8 | DEPLOY-01 | API и Flutter Web production deploy | DB-02 | BLOCKED |
| 9 | QA-01 | Production smoke, security и rollback verification | DEPLOY-01 | TODO |
| 10 | CONTENT-01 | Show-ready brand assets и content pack | DEPLOY-01 | TODO |
| 11 | SHOW-01 | Demo accounts, reset и презентационный сценарий | QA-01, CONTENT-01 | TODO |
...
|---|---|---|---|---|
| 2026-07-16 | DEPLOY-01 | **BLOCKED после частичного concurrent rollout.** Exact SHA `65bae0a1ba07cef8b8f9bb37bea715d3f8636241` успешно pushed в `main`; владелец явно разрешил concurrent API/web rollout. Новый API deploy `dep-d9cg0i8js32c739belsg` — `live`; web deploy `dep-d9cg0i8js32c739belr0` — `build_failed`, поэтому предыдущая web версия остаётся live. Dashboard доступен; approved test user существует, email подтверждён и account не заблокирован. Никаких env changes или миграций в DEPLOY-01 не выполнялось. | Read-only deploy dry-run в Render workspace `My workspace` — PASS: services/URLs/repository/branch подтверждены. API production smoke — PASS: `/health` 200 (`healthy`), `/health/ready` 200 (`production`), tenant bootstrap 200 с BrandConfig «Огороднік Олександр», CORS OPTIONS с web origin 200 и exact `Access-Control-Allow-Origin`. Web build failure evidence: `./scripts/render-build.sh: line 8: SUPPORT_EMAIL: SUPPORT_EMAIL is required for production builds`. Previous healthy rollback baselines: API `dep-d9b4pqeq1p3s73eqeer0` (commit `f239e0c…`), web `dep-d9b5au2b0ehc73bipuk0` (commit `7a75b029…`). | Нужен выбранный владельцем public `SUPPORT_EMAIL` в Render static-site; не выбирать и не устанавливать его самовольно. После env update Render должен повторить web build, затем обязательны web/auth/catalogue/allowlist smoke. Следующая задача остаётся `DEPLOY-01`; API rollback не нужен, потому что новый API healthy. |
