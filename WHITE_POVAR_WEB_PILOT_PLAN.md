...
Следующая задача: `QA-01` — `BLOCKED` до deploy текущего P1-fix, повторной buyer-проверки premium content и завершения оставшихся critical journeys/rollback drill.
...
|---|---|---|---|---|
| 2026-07-19 | QA-01 | **BLOCKED: API demo-purchase P1 подтверждён исправленным на production, но найден и локально исправлен второй P1 доступа к premium content.** После owner-triggered API deploy allowlisted buyer активировал demo-access: UI подтвердил Premium, а server ledger создал ровно один active tenant entitlement. Затем premium collection открылась, однако premium recipe остался locked: consumer gates читали legacy `subscriptionProvider`, тогда как web demo commerce обновляет только server-backed `paywallProvider`. Текущий локальный fix делает shared gate зависимым от server entitlement, refreshes entitlement при создании paywall state и добавляет regression test. Active entitlement сразу отозван, browser reload подтвердил closed collection и locked premium item. | Production preflight `/health` и `/health/ready` = 200; browser activation/entitlement/revoke drill выполнен на выделенном buyer без платежа. Local: targeted Flutter tests `22 passed`; full `flutter test`, `flutter analyze`, release `flutter build web --release` и `git diff --check` PASS. Новый P1-fix ещё не committed/deployed; production web всё ещё содержит mismatch, поэтому нельзя заявлять premium journey PASS. | Commit и push текущего frontend fix, затем deploy web service и повторить buyer activation → premium recipe body → revoke/reload. `QA-01` остаётся BLOCKED также на remaining 17-journey evidence, Studio/free-account flows, one-off offer/scope и reversible config/web rollback drill. Не переходить к `SHOW-01`. |
