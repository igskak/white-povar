...
Следующая задача: `STRIPE-01` — `PARKED` до решения владельца начать Stripe test mode; QA-01 и SHOW-01 завершены.
...
|---|---|---|---|---|
| 2026-07-19 | SHOW-01 | **DONE.** Separate resettable free viewer, allowlisted buyer and Studio-admin are ready. The two production buyer demo paths have each been run and reset during QA: subscription → premium/reload → revoke/locked, and one-off collection → scope audit → revoke/locked. [`SHOW_01_RUNBOOK.md`](SHOW_01_RUNBOOK.md) contains a 7–10 minute sequence, no-charge wording, pre/post-run reset, fallback use, kill switch and feedback prompts; it contains no secrets or passwords. | Role/reset audit: free viewer and Studio admin available; buyer auth identity available; active demo entitlements = 0. Runbook resets through UI plus the internal revoke tool, with no hand SQL editing. Web URLs, brand, free content and premium collection are deployed and verified in QA. | `SHOW-01` = DONE. Next planned package `STRIPE-01` remains PARKED until owner decision. |
