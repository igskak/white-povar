...
|---|---|---|---|---|
| 2026-07-19 | QA-01 | **BLOCKED: первый frontend P1-fix deployed, но post-reload buyer smoke выявил race восстановления auth.** Activation создаёт один active server entitlement и paywall корректно показывает Premium. После full reload premium collection всё ещё locked: initial entitlement read случался до готовности persisted auth session. Локальный follow-up связывает paywall refresh с `authProvider`: authenticated session запускает server entitlement read, sign-out clears in-memory access; добавлены test overrides для isolated paywall suite. Active entitlement сразу отозван. | Production preflight `/health` и `/health/ready` = 200; browser activation/entitlement drill воспроизвёл P1 после web deploy `8bca13f`; server audit подтвердил `1 active`, затем CLI revoke = `1`. Local follow-up: targeted Flutter tests `22 passed`, `flutter analyze` PASS. Новый follow-up ещё не committed/deployed; production web всё ещё не проходит post-reload premium access, поэтому journey не PASS. | Commit/push follow-up, дождаться web deploy и повторить buyer activation → full reload → premium recipe body → revoke/reload. `QA-01` остаётся BLOCKED также на remaining 17-journey evidence, Studio/free-account flows, one-off offer/scope и reversible config/web rollback drill. Не переходить к `SHOW-01`. |
