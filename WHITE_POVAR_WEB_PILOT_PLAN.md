...
Следующая задача: `DEPLOY-01` (BLOCKED: требуется authenticated production smoke через явно предоставленную owner test session; у pilot пока нет конечных пользователей)
