...
Следующая задача: `DEPLOY-01` (BLOCKED: production auth session сбрасывается сразу после successful login transition; требуется corrective fix и повторный authenticated smoke)
