...
class RecipeFilter {
  ...
  bool get isEmpty => cuisine == null && category == null && maxTime == null && isFeatured == null;
}
...
class RecipeListNotifier extends StateNotifier<AsyncValue<List<Recipe>>> {
  ...
  RecipeListNotifier(this._recipeService, this._currentFilter) : super(const AsyncValue.loading());
    ...
    try {
      ...
      if (!_currentFilter.isEmpty) {
        recipes = recipes.where((recipe) {
          if (_currentFilter.cuisine != null && recipe.cuisine != _currentFilter.cuisine) {
            return false;
          }
          if (_currentFilter.category != null && recipe.category != _currentFilter.category) {
            return false;
          }
          if (_currentFilter.maxTime != null && recipe.totalTimeMinutes > _currentFilter.maxTime!) {
            return false;
          }
          return true;
        }).toList();
